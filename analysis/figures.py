...
VERSION_LABEL = {"v1-half": "v1 half dose\n(1.5M tok)", "v1": "v1 (2.9M tok,\n2 ep)",
                 "v3": "v2 packed\n(4.9M, 1 ep)", "v2": "v2 packed\n(9.8M, 2 ep)",
                 "v4": "v4 unpacked\n(4.9M, 1 ep)"}
...
def fig_binding_and_parse(rows):
    ...
    for ax, key, color, title in [
        ...
        ax.set_xticks(list(xs), [VERSION_LABEL[v] for v in order], fontsize=8.5)
        ax.set_ylim(0, 1.12)
...
def fig_honesty(rows):
    ...
    ax.set_xticks(range(len(slots)), slots, fontsize=9)
